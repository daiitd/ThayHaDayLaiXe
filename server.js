 ...
 */
app.post('/api/admin/registrations/:id/delete', authMiddleware, async (req, res) => {
  const id = Number(req.params.id);
  if (!id) return sendError(res, 400, 'Invalid id');
  ...
  try {
    ...
    conn.release();
    sendError(res, 500, 'Server error', e?.message ? String(e.message) : null);
  }
});
 ...
 */
app.post('/api/admin/customers/:id/delete', authMiddleware, async (req, res) => {
  const customerId = Number(req.params.id);
  if (!customerId) return sendError(res, 400, 'Invalid customer_id');
